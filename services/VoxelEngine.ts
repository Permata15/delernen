...
export class VoxelEngine {
  ...
  constructor(
    ...
    this.scene.fog = new THREE.Fog(CONFIG.BG_COLOR, 80, 180); // Pushed fog back to eliminate background haze

    this.camera = new THREE.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 0.1, 1000);
    // Magnified view / camera closer so that the model looks larger and clearer ("gambar diperjelas")
    this.camera.position.set(20, 20, 42);

    this.renderer = new THREE.WebGLRenderer({ antialias: true, powerPreference: "high-performance" });
    this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); // Dynamic physical pixel ratio fallback for sharp borders
    ...
    // Beautiful warm natural light environment - Hemispherical sky/ground light
    const hemiLight = new THREE.HemisphereLight(0xffffff, 0xa0aec0, 0.95);
    ...
    dirLight.shadow.bias = -0.0005; // Drastically minimizes surface shadow acne
    this.scene.add(dirLight);

    // Soft fill light from opposite angle simulating natural sky bounce light
    const fillLight = new THREE.DirectionalLight(0xe0f2fe, 0.7);
    fillLight.position.set(-40, 30, -30);
    this.scene.add(fillLight);

    // Floor - beautiful soft neutral clay surface
    const planeMat = new THREE.MeshStandardMaterial({ color: 0xf1f5f9, roughness: 0.92 });
